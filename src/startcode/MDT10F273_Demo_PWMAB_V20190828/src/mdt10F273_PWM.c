...
程序说明：
		使PWMA0（PC5）/PWMA1（PC4）/PWMB0（PC3）/PWMB1（PC0）输出频率为1KHz，占空比为50%的PWM波。
...
u16 temp_cyc;                    //PWM周期缓存器
u16 temp_duty;                   //PWM占空比缓存器
...
//	T1SYNC = 1;     //Timer1外部时钟输入同步控制位	1: 不同步外部时钟输入
	...
	TMR1H = 0x00;   //timer1计数器/计时器高字节初始化
	TMR1L = 0x00;   //timer1计数器/计时器低字节初始化
...
//	T2SYNC = 1; 	//Timer2外部时钟输入同步控制位		1: 不同步外部时钟输入
	...
	TMR2H = 0x00;	//timer2计数器/计时器高字节初始化
	TMR2L = 0x00;	//timer2计数器/计时器高字节初始化
...
*/
	...
	T2PRLL = temp_cyc & 0x00ff;	//PWMA0/1  周期寄存器的低字节。
	...
	EPRA1H = temp_duty >> 8;    //PWMA0 占空比重载寄存器高4位__b<3:0>
	PRLA1L = temp_duty & 0x00ff;//PWMA0 占空比重载寄存器低字节


	EPRB0H = temp_duty >> 8;    //PWMA0 占空比重载寄存器高4位__b<3:0>
	PRLB0L = temp_duty & 0x00ff;//PWMA0 占空比重载寄存器低字节

	EPRB1H = temp_duty >> 8;    //PWMA0 占空比重载寄存器高4位__b<3:0>
	PRLB1L = temp_duty & 0x00ff;//PWMA0 占空比重载寄存器低字节
	...
	PWMA0SEL_PC5OUT();	//PWMA0选择 由PC5输出		(PWMA0 可以选择 由 PC5或者PA0 之一 输出)
//	PWMA0SEL_PA0OUT();	//PWMA0选择 由PA0输出
//	PWMA0OUTOFF();		//PWMA0禁止输出
	PWMA0OUTON();		//PWMA0使能输出

	PWMA1OL = 0;		//PWMA1 极性输出不反向
	PWMA1SEL_PC4OUT();	//PWMA1选择 由PC4输出		(PWMA1 可以选择 由 PC4或者PA1 之一 输出)
...
*/
	...
	PWMB0SEL_PC3OUT();	//PWMB0选择 由PC3输出		(PWMB0 可以选择 由 PC3或者PA2 之一 输出)
//	PWMB0SEL_PA2OUT();	//PWMB0选择 由PA2输出
//	PWMB0OUTOFF();		//PWMB0禁止输出
	PWMB0OUTON();		//PWMB0使能输出

	PWMB1OL = 0;		//PWMB1 输出极性不反向
	PWMB1SEL_PC0OUT();	//PWMB1选择 由PC0输出		(PWMB1 可以选择 由 PC0或者PA5 之一 输出)
